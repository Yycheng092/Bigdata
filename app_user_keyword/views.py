...
def filter_dataFrame(user_keywords, cond, cate, weeks):
    ...
    # (3) proceed filtering: keywords (with type checking)
    if cond == 'and':
        condition = condition & df.content.apply(
            lambda text: isinstance(text, str) and all(
                qk in text for qk in user_keywords)
        )
    elif cond == 'or':
        condition = condition & df.content.apply(
            lambda text: isinstance(text, str) and any(
                qk in text for qk in user_keywords)
        )
...
print("app_user_keyword--關心關鍵字載入成功!")
