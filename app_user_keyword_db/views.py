...
def api_get_userkey_data(request):
    ...
    same_paragraph = get_same_para(
        queryset, key, cond, k=6)  # multiple keywords
    ...
    key_freq_cat, key_occurrence_cat = count_keyword_with_orm(
        queryset, key)  # OK
...
def filter_database_fullText(user_keywords, cond, cate, weeks):
    ...
    start_date = datetime(2025, 4, 2).date()
    latest_date = datetime(2025, 4, 8).date()

    queryset = NewsData.objects.filter(
        date__gte=start_date, date__lte=latest_date)  # Filter by date range
    # queryset = NewsData.objects.all()  # For testing, remove date filter
...
def count_keyword_with_orm(queryset, query_keywords):
    ...
    category_counts = queryset.values(
        'category').annotate(count=Count('item_id'))
    ...
    for category, articles in category_groups.items():
        ...
        for article in articles:
            ...
            freq = sum([len(re.findall(keyword, article.content, re.I))
                       for keyword in query_keywords])
            category_freq += freq
...
def get_keyword_time_based_freq(queryset):
    ...
    query_freq = pd.DataFrame({'date_index': pd.to_datetime(
        dates), 'freq': [1 for _ in range(len(dates))]})
...
def api_get_userkey_sentiment(request):
    ...
    # Proceed filtering
    df_query = filter_database_fullText(query_keywords, cond, cate, weeks)
...
def get_daily_basis_sentiment_count(df_query, sentiment_type='pos', freq_type='D'):
    ...
    if sentiment_type == 'pos':
        def lambda_function(senti): return 1 if senti >= 0.6 else 0  # 大於0.6為正向
    elif sentiment_type == 'neg':
        def lambda_function(senti): return 1 if senti <= 0.4 else 0  # 小於0.4為負向
    elif sentiment_type == 'neutral':
        def lambda_function(
            senti): return 1 if senti > 0.4 & senti < 0.4 else 0  # 介於中間為中立
    else:
        return None
